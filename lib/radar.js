function init(h,w) {
    ...
    var cData = careerData(data, quadrantColors).createData();
    ...
    .shape(function(d) { return d.movement; })
    ...
    function addLegendItems(quadrant, position) {
        var items = cData.content.slice(quadrant.start, quadrant.end);
    ...
    for (var i = 0; i < cData.quadrants.length; i++) {
        var qleft = 5 + (1020 * (i % 2));
        var qtop = 36 + (500 * (i > 1 ? 1 : 0));
        var quad = cData.quadrants[i];
    ...
    //quadrant lines
    radar.add(pv.Line)
    .data([(h/2-radarArcs[3].r), h-(h/2-radarArcs[3].r)])
    .lineWidth(1)
    .left(w/2)
